...
// For B2B guest users, preferred_username in a v2.0 access token is the #EXT# UPN form
// generated by Entra (e.g. alice_externalorg.gov#EXT#@yourtenant.onmicrosoft.com).
// This reverses the encoding to recover the original external email for pre-approval matching.
// Uses the last underscore as the split point because the domain portion never contains '@'.
function extractEmailFromExtUpn(upn: string): string | null {
  const extIdx = upn.indexOf('#ext#');
  if (extIdx === -1) return null;
  const prefix = upn.substring(0, extIdx); // e.g. "alice_externalorg.gov"
